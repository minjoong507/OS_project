               ...
               thread_func *function, void *aux) 
{
  ...
  /* Allocate thread. */
  t = palloc_get_page (PAL_ZERO); //Kernel level get page
  if (t == NULL)
    return TID_ERROR;
...
thread_exit (void) 
{
  ...
  //jsA
  for(e = list_begin (&running_thread()->children_list);
      e != list_end (&running_thread()->children_list);
      e = list_next(e)){
	sema_up(&running_thread() -> destroy_sema);
  }

  sema_up(&running_thread() ->join_sema); 	/* release parent thread_join() */
  sema_down(&running_thread() ->destroy_sema);	/* wait for thread join */
...
{
    // check this tid is child_tid or not
    bool is_child = false ;
    struct list_elem *e;
    ...
    // wait thread
    sema_down (&child_thread->join_sema);    
    list_remove (&child_thread->children_elem);
    sema_up (&child_thread->destroy_sema);
}
...
init_thread (struct thread *t, const char *name, int priority)
{
  ...
  list_push_back (&running_thread ()->children_list, &t->children_elem);
